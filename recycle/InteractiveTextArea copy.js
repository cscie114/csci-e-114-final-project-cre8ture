...
import { updateGraph } from '../libs/createGraph';
import { retrieveUsername } from '../libs/store/usernames';
import { getDoc, applyChanges, createNewDocWithChanges, getChangesForNewDoc } from '../libs/docManager';
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            import('@automerge/automerge').then(Automerge => {
                ...
                setDocUrl(url);
                const initialDoc = getDoc(url); // Get or initialize the Automerge document
                setDoc(initialDoc);
            });
        }
    }, []);
